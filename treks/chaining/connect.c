...
int main (int ac, char **argv) 
{
	...
	/* Right to left */
	key = 1;
    ret = bpf_update_elem(m4, &key, &p3, BPF_ANY);
    if (ret != 0) {
        printf("4 Failed to add key-value pair %s\n", strerror(errno));
        return -1;
    }

    ret = bpf_update_elem(m3, &key, &p2, BPF_ANY);
    if (ret != 0) {
        printf("5 Failed to add key-value pair %s\n", strerror(errno));
        return -1;
    }

	ret = bpf_update_elem(m2, &key, &p1, BPF_ANY);
    if (ret != 0) {
        printf("6 Failed to add key-value pair %s\n", strerror(errno));
        return -1;
    }
